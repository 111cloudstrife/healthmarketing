jQuery(function($) {
	...
	if ($('body').hasClass('course')) {
		...
		$('.step-checkbox').click(function(e) {
			...
			$.ajax({
				type: "POST",
				data: { cmd: 'update_course_steps', step: $(this).attr('step'), action: action },
				dataType: 'text',
				success: function( response ) {
					if (action == 'complete') {
						$(checkbox).removeClass('incomplete fa-square-o').addClass('complete fa-check-square-o');
					} else {
						$(checkbox).addClass('incomplete fa-square-o').removeClass('complete fa-check-square-o');
					}
